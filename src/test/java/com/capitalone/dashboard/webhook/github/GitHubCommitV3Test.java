...
public class GitHubCommitV3Test {
    ...
    public void getCommitsTest() throws HygieiaException, ParseException {
        ...
        when(collectorService.createCollector(any())).thenReturn(collector);
        when(gitHubCommitV3.buildCollectorItem(any(), anyString(), anyString())).thenReturn(collectorItem);
        when(collectorService.createCollectorItem(any())).thenReturn(collectorItem);
        try {
        ...
        try {
            when(gitHubCommitV3.getCommitNode(any(), anyString(), anyString(), any(), anyString())).thenReturn(null);
        } catch (Exception e) {
        ...
        verify(gitHubCommitV3, times(3)).getCommitNode(any(), anyString(), anyString(), any(), anyString());
    ...
    public void setCollectorItemIdExistingCommitTest() throws MalformedURLException, HygieiaException {
        ...
        when(commitRepository.findAllByScmRevisionNumberAndScmUrlIgnoreCaseAndScmBranchIgnoreCaseOrderByTimestampAsc(any(), any(), any())).thenReturn(commitList);
    ...
    public void setCollectorItemIdNewCommitTest() throws MalformedURLException, HygieiaException {
        ...
        when(commitRepository.findAllByScmRevisionNumberAndScmUrlIgnoreCaseAndScmBranchIgnoreCaseOrderByTimestampAsc(any(), any(), any())).thenReturn(null);
        when(collectorService.createCollector(any())).thenReturn(collector);
        when(gitHubCommitV3.buildCollectorItem(any(), anyString(), anyString())).thenReturn(collectorItem);
        when(collectorService.createCollectorItem(any())).thenReturn(collectorItem);
        try {
    ...
    public void getCommitNodeTest() {
        ...
        try {
            when(restClient.parseAsObject(any())).thenReturn(responseJsonObject);
        } catch (Exception e) {
            LOG.error(e.getMessage());
        }
    ...
    public void getRepositoryTokenTest() {
        ...
        when(collectorService.createCollector(any())).thenReturn(collector);
        when(gitHubCommitV3.getCollectorItemRepository().findAllByOptionNameValueAndCollectorIdsIn(any(), any(), any())).thenReturn(gitHubRepoList);
    ...
    private static String createGuid(String hex) {
        ...
        IntStream.range(0, bytes.length).forEach(j -> {
            int v = bytes[j] & 0xFF;
