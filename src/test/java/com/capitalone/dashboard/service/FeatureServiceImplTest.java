...
//				String sProjectID, String sProjectChangeDate, String sProjectBeginDate, String sProjectEndDate)
	...
	public void testGetFeatureEstimates_ManySameSuperFeatures_OneSuperFeatureRs() {
		...
		when(featureRepository.findByActiveEndingSprints(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.anyString(), ArgumentMatchers.anyBoolean()))
	...
	public void testGetCurrentSprintDetail_ValidKanbanTeam_ShowKanban() {
		...
		when(featureRepository.findByUnendingSprints(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.anyBoolean()))
	...
	public void testGetRelevantStories_Kanban() {
		when(componentRepository.findOne(mockComponentId)).thenReturn(mockComponent);
		when(collectorRepository.findOne(mockItem3.getCollectorId())).thenReturn(mockJiraCollector);
		when(featureRepository.findByUnendingSprints(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.anyBoolean())).thenReturn(Arrays.asList(mockJiraFeature3_oldkanban, mockJiraFeature4_sprintkanban));
		when(featureRepository.findByNullSprints(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.anyBoolean())).thenReturn(Arrays.asList(mockJiraFeature5_nullkanban));
	...
	public void testGetRelevantStories_Scrum() {
		when(componentRepository.findOne(mockComponentId)).thenReturn(mockComponent);
		when(collectorRepository.findOne(mockItem3.getCollectorId())).thenReturn(mockJiraCollector);
		when(featureRepository.findByActiveEndingSprints(ArgumentMatchers.anyString(), ArgumentMatchers.anyString(), ArgumentMatchers.any(), ArgumentMatchers.anyString(), ArgumentMatchers.anyBoolean())).thenReturn(Arrays.asList(mockJiraFeature, mockJiraFeature2));
	...
	private Feature createFeature(ObjectId collectorId, String sId, String sStatus, String sState, Integer sEstimate, String changeDate, boolean isDeleted, 
		...
		rt.setsEstimateTime(sEstimate * 60 * 10);
