...
public class DashboardRemoteRequest {
    ...
    public static abstract class Entry {
        ...
        public CollectorItem toCollectorItem(Collector collector) throws HygieiaException{
            if (options.keySet().containsAll(collector.getUniqueFields().keySet())) {
                ...
                collectorItem.setPushed(pushed);
                ...
                for (Map.Entry<String, Object> stringObjectEntry : options.entrySet()) {
                    if (collector.getAllFields().keySet().contains(stringObjectEntry.getKey())) {
                        collectorItem.getOptions().put(stringObjectEntry.getKey(), stringObjectEntry.getValue());
                    } else {
                        throw new HygieiaException(toolName + " collector does not support field: " + stringObjectEntry.getKey(), HygieiaException.COLLECTOR_ITEM_CREATE_ERROR);
                    }
                }
    ...
    public static class FeatureEntry extends Entry {
        ...
        public Map<String, Object> toWidgetOptions() {
            ...
            options.keySet().forEach(key -> opts.put(key, options.get(key)));
            return opts;
        }
    }
