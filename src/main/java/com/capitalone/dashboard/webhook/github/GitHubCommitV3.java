...
public class GitHubCommitV3 extends GitHubV3 {
    ...
    private static final Pattern COMPILE_REPLACE = Pattern.compile("refs/heads/", Pattern.LITERAL);

    private final CommitRepository commitRepository;
    private final GitRequestRepository gitRequestRepository;
    private final CollectorItemRepository collectorItemRepository;

    private final Map<String, String> ldapDNMap = new HashMap<>();
    ...
    public String process(JSONObject jsonObject) throws MalformedURLException, HygieiaException, ParseException {
        ...
        if (!StringUtils.isEmpty(ref)) {
            branch = COMPILE_REPLACE.matcher(ref).replaceAll(Matcher.quoteReplacement(""));
        }
    ...
    protected static void setCommitPullNumbersForRebaseAndMergeCommit(List<Commit> commitsList) {
        ...
        if (checkCommitsWithPullNumber(commitsWithPullNumber)
            ...
            Objects.requireNonNull(commitsList).forEach(commit -> commit.setPullNumber(commitWithPR.getPullNumber()));
        }
    }

    private static boolean checkCommitsWithPullNumber(List<Commit> commitsWithPullNumber) {
        return !CollectionUtils.isEmpty(commitsWithPullNumber)
                && (commitsWithPullNumber.size() == 1);

    }

    private static boolean checkCommitsListForSettingPullNumber(List<Commit> commitsList) {
    ...
    protected Object getCommitNode(GitHubParsed gitHubParsed, String branch, String commitId,
        ...
        JSONObject postBody = getQuery(gitHubParsed, branch, timeStamp.toString());
    ...
    protected JSONObject getQuery(GitHubParsed gitHubParsed, String branch, String timeStamp) {
        ...
        query.put("query", GraphQLQuery.COMMITS_GRAPHQL);
