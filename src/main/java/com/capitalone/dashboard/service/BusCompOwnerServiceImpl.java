...
public class BusCompOwnerServiceImpl implements BusCompOwnerService{
     ...
     */
    private static Owner getUserInformation(Authentication authentication){
        AuthType authType = (AuthType)authentication.getDetails();

        return new Owner(authentication.getName(), authType);
    }
    /**
    ...
    private List<String> getBusinessServiceList(String firstName, String lastName){
         ...
         */
        Predicate<Cmdb> supportOwnerLn = p -> p.getSupportOwner() != null && p.getSupportOwner().toLowerCase().contains(lastName);
        Predicate<Cmdb> serviceOwnerLn = p -> p.getAppServiceOwner() != null && p.getAppServiceOwner().toLowerCase().contains(lastName);
        Predicate<Cmdb> developmentOwnerLn = p -> p.getDevelopmentOwner() != null && p.getDevelopmentOwner().toLowerCase().contains(lastName);
        Predicate<Cmdb> businessOwnerLn = p -> p.getBusinessOwner() != null && p.getBusinessOwner().toLowerCase().contains(lastName);
        /**
     ...
     */
    private static boolean doesMatchFullName(String firstName, String fullName){
        ...
        if(firstName != null && !firstName.isEmpty() && fullName != null && !fullName.isEmpty()){
            ...
            if(firstFromCMDB.length() < firstName.length()){
                if(firstName.contains(firstFromCMDB)){
                    matching = true;
                }
            }else if (firstFromCMDB.contains(firstName)){
                matching = true;
            }
        }
        return matching;
