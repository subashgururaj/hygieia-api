...
public class DynamicPipelineServiceImpl implements PipelineService {
    ...
    private static final Comparator<Build> BUILD_NUMBER_COMPATATOR = (o1, o2) -> {
    ...
    public DynamicPipelineServiceImpl(PipelineRepository pipelineRepository, DashboardRepository dashboardRepository,
									  CollectorItemRepository collectorItemRepository, @Qualifier("binaryArtifactServiceImpl") BinaryArtifactService binaryArtifactService,
    ...
    private PipelineResponse buildPipelineResponse(Pipeline pipeline, Long lowerBound, Long upperBound){
		...
		List<PipelineStage> pipelineStageList = new ArrayList<>(stageToEnvironmentNameMap.keySet());
    ...
    protected Pipeline buildPipeline(Pipeline pipeline, Long lowerBound, Long upperBound) {
        ...
        Map<ArtifactIdentifier, Collection<BinaryArtifact>> artifacts = getBinaryArtifacts(
        		environmentArtifactIdentifiers.values().stream().flatMap(Collection::stream).collect(Collectors.toList()));
    	...
    	builds.sort(BUILD_NUMBER_COMPATATOR);
	...
	protected void processCommits(Pipeline pipeline, List<Commit> commits) {
    	...
    	if (logger.isDebugEnabled()) {
    		String sb = commits.stream().map(commit -> "    - " + commit.getId() + " (" + commit.getScmRevisionNumber() + ") - " + commit.getScmCommitLog() + "\n").collect(Collectors.joining("", "\n===== Commit List =====\n", ""));
    ...
    protected void processBuilds(Pipeline pipeline, List<Build> builds, List<Commit> commits) {
    	...
    	sortedBuilds.sort(BUILD_NUMBER_COMPATATOR);
     ...
     * Given an {@link Environment} this method will iterate over its until
     ...
     * @return				the filtered list
     */
    protected static Map<ArtifactIdentifier, Collection<BinaryArtifact>> filterBinaryArtifacts(Map<ArtifactIdentifier, Collection<BinaryArtifact>> artifactsMap, String url, String branch) {
    	Map<ArtifactIdentifier, Collection<BinaryArtifact>> rt = new HashMap<>();
    	String urlNoNull = url != null? url : "";
    	String branchNoNull = branch != null? branch : "";

		artifactsMap.forEach((id, value) -> {
			List<BinaryArtifact> artifacts = new ArrayList<>();

			boolean added = false;
			for (BinaryArtifact ba : value) {
     ...
     */
	protected static RepoBranch getComponentRepoBranch(Component component) {
	...
	private static Map<Environment, Collection<ArtifactIdentifier>> getArtifactIdentifiers(List<Environment> environments) {
	...
	private List<BinaryArtifact> getBinaryArtifacts(String group, String name, String version, String ext) {
		...
		request.setArtifactExtension(ext); // empty string extension is valid
	...
	private static Multimap<ObjectId, Commit> buildBuildToCommitsMap(List<Build> builds, List<Commit> commits) {
		...
		builds.forEach(build -> build.getSourceChangeSet().stream().map(SCM::getScmRevisionNumber).map(revisionNumberToCommitMap::get).filter(Objects::nonNull).forEach(correspondingCommit -> {
	...
	private static Map<String, Commit> buildRevisionNumberToCommitMap(List<Commit> commits) {
	...
	protected static List<String> getCommitHistory(Map<String, Collection<String>> commitGraph, String headRevisionNumber) {
	...
	private static void getCommitHistory(List<String> rt, Set<String> seenRevisions, Map<String, Collection<String>> commitGraph, String revisionNumber) {
		...
		commitGraph.get(revisionNumber).stream().filter(seenRevisions::add).forEach(rn -> {
			rt.add(rn);
     ...
     * @return
     */
    private static PipelineResponseCommit applyStageTimestamps(PipelineResponseCommit commit, Dashboard dashboard, Pipeline pipeline, List<PipelineStage> pipelineStageList){
		...
		pipelineStageList.forEach(systemStage -> {
     ...
     * @return
     */
    private static Map<String, PipelineCommit> findCommitsForStage(Dashboard dashboard, Pipeline pipeline, PipelineStage stage) {
     ...
     */
    public static List<PipelineResponseCommit> findNotPropagatedCommits(Dashboard dashboard, Pipeline pipeline, PipelineStage stage, List<PipelineStage> pipelineStageList){

        Map<String, PipelineCommit> startingStage = findCommitsForStage(dashboard, pipeline, stage);
		return startingStage.entrySet().stream().map(entry -> applyStageTimestamps(new PipelineResponseCommit(entry.getValue()), dashboard, pipeline, pipelineStageList)).collect(Collectors.toList());
    }
