...
public class DashboardServiceImpl implements DashboardService {
    ...
    public Dashboard get(ObjectId id) {
        ...
        if (!dashboard.getApplication().getComponents().isEmpty()) {
            ...
            itemMap.values().stream().flatMap(Collection::stream).forEach(collectorItem -> collectorItem.setCollector(getCollector(collectorItem.getCollectorId(), collectors)));
        }
    ...
    public void delete(ObjectId id) {
        ...
        //NOPMD - using fully qualified or we pickup an incorrect spring class
        serviceRepository.findByDependedBy(id).forEach(service -> {
    ...
    private void handleCollectorItems(List<Component> components) {
        components.stream().map(Component::getCollectorItems).forEach(itemMap -> itemMap.forEach((key, items) -> items
                .stream()
                .filter(i -> CollectionUtils.isEmpty(componentRepository.findComponents(i.getCollectorId(), key, i)))
                .forEach(i -> {
            i.setEnabled(false);
    ...
    public Component associateCollectorToComponent(ObjectId componentId, List<ObjectId> collectorItemIds) {
        ...
        collectorItemIds.forEach(collectorItemId -> {
        ...
        Set<CollectorItem> deleteSet = new HashSet<>(toSaveCollectorItems.values());
    ...
    private boolean isLonely(CollectorItem item, Collector collector, Component component) {
        List<Component> components = componentRepository.findComponents(collector, item);
    ...
    private Iterable<Collector> collectorsFromItems(Map<CollectorType, List<CollectorItem>> itemMap) {
        ...
        itemMap.values().forEach(collectorItems -> collectorItems.stream().map(CollectorItem::getCollectorId).forEach(collectorIds::add));

        return collectorRepository.findAllById(collectorIds);
    }

    private static Collector getCollector(final ObjectId collectorId, Iterable<Collector> collectors) {
        return Iterables.tryFind(collectors, collector -> Objects.requireNonNull(collector).getId().equals(collectorId)).orNull();
    }
    ...
    public List<ObjectId> getOwnedDashboardsObjectIds() {
        return getOwnedDashboards().stream().map(BaseModel::getId).collect(Collectors.toList());
    }
    @Override
    ...
    public  List<Dashboard> getByTitle(String title) {
        ...
        return dashboardRepository.findByTitle(title);
    }
    ...
    public Dashboard updateDashboardWidgets(ObjectId dashboardId, Dashboard request) throws HygieiaException {
        ...
        widgetToDelete.forEach(widgetName -> widgets.stream().filter(widget -> widgetName.equalsIgnoreCase(widget.getName())).mapToInt(widgets::indexOf).forEach(widgetIndex -> {
        ...
        indexList.forEach(i -> widgets.set(i, null));
        updatedWidgets = widgets.stream().filter(Objects::nonNull).collect(Collectors.toList());
        ...
        if(componentId!=null){
            ...
            collectorTypesToDelete.forEach(cType -> component.getCollectorItems().remove(cType));
            componentRepository.save(component);
        }
        return dashboard;
    }
    ...
    public void deleteWidget(Dashboard dashboard, Widget widget,ObjectId componentId) {
        ...
        List<Widget> widgets = dashboard.getWidgets();
        List<Widget> updatedWidgets = widgets.stream().filter(Objects::nonNull).collect(Collectors.toList());
        dashboard.setWidgets(updatedWidgets);
        dashboardRepository.save(dashboard);
        ...
        if(componentId!=null){
            ...
            collectorTypesToDelete.forEach(c -> component.getCollectorItems().remove(c));
    ...
    private static List<String> findUpdateCollectorItems(List<String> existingWidgets, List<String> currentWidgets){
        return existingWidgets.stream().filter(elem -> !currentWidgets.contains(elem)).collect(Collectors.toList());
    }
    ...
    private void getAppAndComponentNames(List<Dashboard> findByOwnersList) {
        findByOwnersList.forEach(dashboard -> {
            String appName = dashboard.getConfigurationItemBusServName();
            String compName = dashboard.getConfigurationItemBusAppName();
            setAppAndComponentNameToDashboard(dashboard, appName, compName);
        });
