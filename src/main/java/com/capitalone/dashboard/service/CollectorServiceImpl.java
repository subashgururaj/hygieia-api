...
public class CollectorServiceImpl implements CollectorService {
    ...
    public List<Collector> collectorsById(ObjectId id) {
        ...
        return collector != null ? Lists.newArrayList(collector) : Collections.emptyList();
    }
    ...
    public Page<CollectorItem> collectorItemsByTypeWithFilter(CollectorType collectorType, String searchFilterValue, Pageable pageable) {
        ...
        if(!StringUtils.isEmpty(searchFilter.getAdvancedSearchKey()) && (criteria != null ? criteria.size() : 0) >1){
    ...
    public CollectorItem createCollectorItemSelectOptions(CollectorItem item, Map<String, Object> allOptions, Map<String, Object> uniqueOptions) {
        ...
        List<CollectorItem> existing = collectorItemRepository.findCollectorItemsBySubsetOptions(
    ...
    public List<CollectorItem> getCollectorItemForComponent(String id, String type) {
        ...
        List<ObjectId> ids = items.stream().map(BaseModel::getId).collect(Collectors.toList());
        return (List<CollectorItem>) collectorItemRepository.findAllById(ids);
    }

    private static Collector collectorById(ObjectId collectorId, List<Collector> collectors) {
        return collectors.stream().filter(collector -> collector.getId().equals(collectorId)).findFirst().orElse(null);
    }
    ...
    private static String getAdvSearchField(List<String> searchList) {
        return searchList!=null && searchList.size()>1?searchList.get(1):null;
    }

    private static String getDefaultSearchField(List<String> searchList) {
        return searchList!=null?searchList.get(0):null;
    }

    private static List<String> getSearchFields(List<Collector> collectors){
