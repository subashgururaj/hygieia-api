...
public class AuthProperties {
	...
	private static final Pattern COMPILE_REPLACE = Pattern.compile("-", Pattern.LITERAL);
	...
	public void applyDefaultsIfNeeded() {
		if (secret == null) {
			...
			secret = COMPILE_REPLACE.matcher(UUID.randomUUID().toString()).replaceAll(Matcher.quoteReplacement(""));
		}

		if (expirationTime == null) {
			LOGGER.info("No JWT expiration time found in configuration, setting to one day.");
			expirationTime = (long) 1000 * 60 * 60 * 24;
		}
