    ...
    public ResponseEntity<Object> handleHygieiaException(HygieiaException e) {
        ...
        switch (e.getErrorCode()) {
            ...
            case HygieiaException.COLLECTOR_ITEM_CREATE_ERROR:
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(("Internal error." + e.getMessage() + "Error code=: " + e.getErrorCode()));
            case HygieiaException.DUPLICATE_DATA:
            case HygieiaException.JSON_FORMAT_ERROR:
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(("Bad request. " + e.getMessage() + "Error code=: " + e.getErrorCode()));
            case HygieiaException.NOTHING_TO_UPDATE:
                return ResponseEntity.status(HttpStatus.NOT_MODIFIED).body(("Internal error. " + e.getMessage() + "Error code=: " + e.getErrorCode()));
            default:
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(("Internal error. " + e.getMessage() + "Error code=: " + e.getErrorCode()));
        }
    }
